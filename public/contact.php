...
if ($formType === 'contact') {
    ...
    if (strlen($name) < 2) respond(false, 'Name ist zu kurz', 400);
    if ($mode === '') respond(false, 'Modus fehlt', 400);
    if (!is_array($subjects) || count($subjects) === 0) respond(false, 'Bitte waehle mindestens ein Fach', 400);

    $cleanSubjects = array_values(array_filter(array_map(function ($subject) {
        return trim((string) $subject);
    }, $subjects)));

    if (count($cleanSubjects) === 0) respond(false, 'Bitte waehle mindestens ein Fach', 400);
    ...
    $jobTitle = trim($data['jobTitle'] ?? 'Initiativbewerbung');
    $availability = trim($data['availability'] ?? '');
    $subjects = $data['subjects'] ?? [];

    if (strlen($fullName) < 2) respond(false, 'Name ist zu kurz', 400);
    if ($status === '') respond(false, 'Status fehlt', 400);
    if (!is_array($subjects) || count($subjects) === 0) respond(false, 'Bitte trage mindestens ein Fach ein', 400);
    ...
    if (count($subjectLines) === 0) respond(false, 'Bitte trage mindestens ein gueltiges Fach ein', 400);
